...
//            cellIndex.CellIndexMethod().cellIndexMethod(particulas, n, l, m, rc, true)
//
...
//            }
        ...
        fun getVariance(data: DoubleArray): Double {
        ...
        private fun calcularTiempos() {
            ...
            val borderless = true
            val maxM = 25
            val times = Array(maxM) { DoubleArray(iteraciones) }
            ...
            while (i < iteraciones) {
                ...
                for (m in 1..maxM) {
                    val start = System.nanoTime().toDouble()  / 1000000
                    CIM.cellIndexMethod(particulas, n, l, m, rc, borderless)
                    val time = System.nanoTime()  / 1000000 - start
                    times[m-1][i] = time
                }
                i++
            }
            try {
                BufferedWriter(OutputStreamWriter(
                    ...
                    for (m in 1..maxM) {
                        writer.write(m.toString() + "," + getMean(times[m-1]) + "," + getVariance(times[m-1]) + "\n")
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
